Here is your step-by-step guide to setting up the automated plugin generator for `blah-community-untrusted`.
...
3. Name it something like "Blah Plugin Automator".
...
1. Inside `blah-community-untrusted`, create a new public repository named `plugin-requests`.
2. Go to the Settings of this new `plugin-requests` repo -> **Secrets and variables** -> **Actions**.
...
4. Add a label called `plugin-request` in github repo via repo -> **Issues** -> **Labels** -> **New Label**


**Create the Issue Form UI:**
We use YAML to build a strict UI form so users can't mess up the formatting.
1. In the `plugin-requests` repository, create a file at this exact path `.github/ISSUE_TEMPLATE/new-plugin.yml`.
...
1. In the same `plugin-requests` repository, create a file at `.github/workflows/generator.yml`.
Or just copy the one from this repo.
...
1. A developer visits `blah-community-untrusted/plugin-requests`.
2. They click "Issues" -> "New Issue". They see a beautiful, polished UI form asking for a name and description.
3. They submit it.
4. The Action wakes up, verifies the data, securely creates `plugin-[their-name]`, shoots them an email invite to take control of it, and closes the request issue automatically.
